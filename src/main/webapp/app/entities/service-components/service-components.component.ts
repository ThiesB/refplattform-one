...
@Component({
    selector: 'jhi-service-components',
    templateUrl: './service-components.component.html'
})
export class ServiceComponentsComponent implements OnInit, OnDestroy {
    ...
    trackId(index: number, item: ServiceComponents) {
        return item.id;
    }
    registerChangeInServiceComponents() {
        this.eventSubscriber = this.eventManager.subscribe('serviceComponentsListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.serviceComponents.push(data[i]);
        }
    }
